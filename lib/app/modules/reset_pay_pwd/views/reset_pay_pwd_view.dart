...
class ResetPayPwdView extends GetView<ResetPayPwdController> {
  const ResetPayPwdView({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: KWhiteColor,
      appBar: AppBar(title: const Text('重置支付密码'), centerTitle: true),
      // body: Obx(() => controller.show.value ? buildShow() : _buildNew()),
      body: Obx(() {
        if (controller.show.value == 0) {
          return _buildCurrent();
        } else if (controller.show.value == 1) {
          return _buildSet();
        } else {
          return buildReset();
        }
  ...
  Column _buildCurrent() {
    return Column(
      children: [
        Text('输入当前支付密码, 以验证身份', style: TextStyle(fontSize: 18.sp))
            .paddingSymmetric(vertical: 20.h),
        PinInputTextField(
          controller: controller.nowTextEditingController,
          pinLength: 6,
          ...
          onChanged: (pin) {
            controller.onChangeNow(pin);
          },
          enableInteractiveSelection: false,
          cursor: Cursor(
            width: 2,
            color: Colors.grey,
            radius: Radius.circular(7.w),
            enabled: true,
          ),
